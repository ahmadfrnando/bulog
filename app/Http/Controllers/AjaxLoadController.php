...
class AjaxLoadController extends Controller
{
    ...
    //     $data = \App\Models\Siswa::with('user')->where('nama', 'LIKE', '%' . $searchTerm . '%')
    //         ->get(['id', 'nama', 'user_id'])
    //         ->map(function ($item) {
    //             return [
    //                 'id' => $item->id,
    //                 'text' => $item->nama . ' - ' . $item->user->username
    //             ];
    //         })
    //         ->toArray();
    ...
    {
        $searchTerm = $request->input('q');
        $data = \App\Models\Pasar::where('nama', 'LIKE', '%' . $searchTerm . '%')
            ->get(['id', 'nama'])
            ->toArray();
    ...
    {
        $searchTerm = $request->input('q');
        $data = \App\Models\User::where('name', 'LIKE', '%' . $searchTerm . '%')
            ->where('role','petugas')
            ->get(['id', 'name'])
            ->toArray();
